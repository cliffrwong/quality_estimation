...
def attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    stateForIn = [None] * len(initial_for_state)
    ...
    for i in range(len(initial_for_state)):
        with variable_scope.variable_scope("F_init_for_%d" % i):
            ...
            stateForIn[i] = tf.nn.relu(linear(initial_for_state[i], state_size, True, scope="Linear0"), name="relu0")
            # state[i] = tf.nn.relu(linear(y, state_size, True, scope="Linear1"), name="relu1")
    stateForOut = for_cell.zero_state(batch_size, dtype)
    ...
    with variable_scope.variable_scope("Decoder_For"):
        for i, inp in enumerate(reversed(decoder_inputs)):
            if i > 0:
    ...
    stateBacIn = [None] * len(initial_bac_state)
    for i in range(len(initial_bac_state)):
        with variable_scope.variable_scope("F_init_bac_%d" % i):
            ...
            stateBacIn[i] = tf.nn.relu(linear(initial_bac_state[i], state_size, True, scope="Linear0"), name="relu0")
    
    stateIn = stateForIn + stateBacIn
            # state[i] = tf.nn.relu(linear(y, state_size, True, scope="Linear1"), name="relu1")
    # state = bac_cell.zero_state(batch_size, dtype)
    # bac_output = []
    stateBacOut = for_cell.zero_state(batch_size, dtype)
    ...
    with variable_scope.variable_scope("Decoder_Back"):
        ...
        for i, inp in enumerate(decoder_inputs):
            if i > 0:
                variable_scope.get_variable_scope().reuse_variables()
            # inp_concat = tf.concat(1,[inp, context])
            cell_output, stateBacOut = bac_cell(inp, stateBacOut)
            # bac_output.insert(0, cell_output)
    
    stateOut = tf.reshape(tf.concat(0, [stateForOut, stateBacOut]), [1, 5100])
    # print('shape', tf.shape(stateOut))
    stateIn = tf.concat(1, stateIn)
    # q_vec = []
  ...
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(
        ...
        stateIn, stateOut, _ = qualvec(encoder_inputs[:bucket[0]],
                                    decoder_inputs[:bucket[1]+1])
        stateIns.append(stateIn)
        stateOuts.append(stateOut)
